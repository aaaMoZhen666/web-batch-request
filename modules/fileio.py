...
def import_target_file(window):
    """导入目标数据文件内容"""
    try:
        file_types = ('Text Files (*.txt)',)
        file_path = window.create_file_dialog(webview.FileDialog.OPEN, allow_multiple=False, file_types=file_types)[0]
...
def export_result_file(window, data):
    """导出结果数据文件"""
    try:
        file_types = (
            'CSV Files (*.csv)',
